...
class CardvalidatorValidationException implements Exception {
  final List<String> errors;

  CardvalidatorValidationException(this.errors);

  @override
  String toString() => 'CardvalidatorValidationException: ${errors.join("; ")}';
}
...
/// Card Validator API client.
///
/// For more information, visit: https://apiverve.com/marketplace/cardvalidator?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [number] (required) - The card number to validate [minLength: 13, maxLength: 19]
class CardvalidatorClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'number': ValidationRule(type: 'string', required: true, minLength: 13, maxLength: 19),
  };

  CardvalidatorClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/cardvalidator',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [CardvalidatorValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw CardvalidatorValidationException(errors);
    }
  }

  /// Execute a request to the Card Validator API.
  ///
  /// Parameters are validated before sending the request.
  Future<CardvalidatorResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw CardvalidatorException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return CardvalidatorResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw CardvalidatorException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw CardvalidatorException('Resource not found');
      } else {
        throw CardvalidatorException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is CardvalidatorException) rethrow;
      throw CardvalidatorException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Card Validator API client.
class CardvalidatorException implements Exception {
  final String message;

  CardvalidatorException(this.message);

  @override
  String toString() => 'CardvalidatorException: $message';
}
